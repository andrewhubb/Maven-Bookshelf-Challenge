...
#Functions
def load_sentiment_model_manual():
...
def process_chunk_manual(chunk_id, chunk_df, tokenizer, model, batch_size=24):
...
# 🧩 Main Runner
def run_sentiment_pipeline_manual(df, chunk_size=1000):
    tokenizer, model = load_sentiment_model_manual()
    ...
    for chunk_id, chunk_df in chunk_dataframe(df, chunk_size):
        ...
        process_chunk_manual(chunk_id, chunk_df, tokenizer, model)
...
def process_and_save_chunks(df, text_column, chunk_size=1000, batch_size=16, output_dir="emotion_chunks", df_name="works"):
    os.makedirs(output_dir, exist_ok=True)
    total_rows = len(df)

    # Detect already processed chunks by filename
    processed_starts = {
        int(f.split("_")[2])  # e.g. works_emotions_0_1000.parquet → 0
        for f in os.listdir(output_dir)
        if f.startswith(f"{df_name}_emotions_") and f.endswith(".parquet")
    }

    print("✅ Starting chunk processing...")
    print(f"Total rows: {len(df)}")

    for start in tqdm(range(0, total_rows, chunk_size), desc=f"Processing {df_name} chunks"):
        if start in processed_starts:
            continue  # Skip already processed chunk

        end = min(start + chunk_size, total_rows)
        chunk = df.iloc[start:end].copy()
        print(f"[{datetime.now()}] Processing {df_name} rows {start} to {end}...")

        chunk_scores = batched_emotion_scores(chunk[text_column].tolist(), batch_size=batch_size)
        chunk['emotion_scores'] = chunk_scores

        output_path = os.path.join(output_dir, f"{df_name}_emotions_{start}_{end}.parquet")
...
run_sentiment_pipeline_manual(reviews,CHUNK_SIZE)
